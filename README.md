...
12a. **`dist/cloud_config.json` 是真密钥、不进 git，且常常只存在于 `dist/` ——
    每次构建会自动回抄到仓库根做备份。** 两层保护缺一不可：
    - 构建**开头**把 `dist/cloud_config.json` 回抄到仓库根（已 gitignore，且它
      本来就是构建的来源之一，语义不变）。管的是"dist 整个消失"。
    - 构建**中段**再从上一版 `dist/` 抄回暂存目录，优先级最高。管的是"整目录
      替换把手放的配置删了"（软件会静默回落 Mock 假设备，"界面全过实则没连云"
      最难察觉）。
    - 只有前者不够：整目录替换时根目录那份可能是旧的；只有后者不够：dist 一旦
      整个没了就无处可抄 —— 2026-08-20 实际丢过一次，账号密码只在命令行传过、
      无处可查，只能重新问。
    - `factory_config.json` 不回抄仓库根：它的来源是 `resources/`，回抄会造出一个
      永不被读的文件、还会变成未跟踪脏文件；丢了只是退回默认勾选。
13a. **构建前必须关掉程序 —— 不只是因为 exe 被锁，更因为 `dist/` 整体动不了。**
    Windows 下目录内**只要有一个打开的句柄就不允许 rename**，程序跑着时
    `dist/logs/*.log` 正被写，替换 dist 就报 `Device or resource busy`。
    - 第 13 条那个 exe 占用检查挡不住这个：它只测 exe，测不到 logs；而且检查与
      真正替换之间隔着几十秒的构建时间，中间完全可能有人把程序起起来。
    - 所以替换逻辑把「把旧 dist 重命名到 `dist.old.$$`」当成**无损的占用探测**：
      成功就继续（失败还能回滚），失败就立刻停下、一个字节都不动。
    - ⚠️ **不要给它加 `rm -rf dist` 的退路。** 实测 `rm -rf` 同样删不掉被独占的
      文件，只会把 dist 删残了才失败 —— 比直接停下来坏得多。2026-08-20 踩过：
      程序在跑、日志被写着，旧 dist 被删到只剩 1 个文件，只能整包重建。
    - 暂存目录 `dist.stage.$$` 有 `trap ... EXIT INT TERM` 收尾，并在每次构建
      开始时扫掉别的 PID 留下的孤儿 —— 构建被打断时它是**整包大小（357MB）**，
      而下次构建 PID 不同、清不到它，攒几次就吃掉几个 G。
...
30. **多台工装卡：名单来自云端，PC 不需要三元组；选择器必须做成不占高度的下拉。**
    ...
    - `device_secret` 是**设备**连云用的凭据；PC 走 CAM 密钥直接调云 API，只要
      `product_id` 就能查出该产品下全部设备。所以名单不用手工维护，也不必读 SD 卡。
    - ⚠️ `DescribeDevices` 只用于拿名单：2026-09-01 用当前账号实测，其返回的每台
      `Status` 都是 `null`。实时状态必须再并发调用单台 `DescribeDevice`；官方枚举为
      `0`=离线、`1`=在线、`2`=获取失败、`3`=未激活。获取失败或字段异常在界面显示
      “状态未知”并禁选；“未激活”单独显示，因为它通常意味着三元组没写对，和
      “设备没上电”是两种问题。不能沿用 `readDeviceData` 的心跳年龄冒充云端状态。
