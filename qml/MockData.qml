...
QtObject {
    ...
    // =0x6E5:指示灯(0)/白光(2)/电池(5)/云台(6)/喇叭(7)/4G(9)/SD(10)。
    // 无 bit1(红外)与 bit3(日夜) —— 本产品无此硬件。
    // 云台(6)无电机,但设备端先注册桩执行体恒回成功(用户定稿 2026-08-18),
    // 所以算"支持";咪头(8)/复位按键(4)不在其中也无妨 —— 两项已定走
    // "无指令纯人工",不查能力位,见 manualChecks.noCommand。
    readonly property int supportedItems: 0x6E5

    // ---- 人工判定条目全集(8 条) ----
    // 人工项位号(指示灯0/白光2/复位4/云台6/喇叭7/咪头8):发光/出声/转动/手感
    // 固件感知不到,设备最多回"指令已执行",现象只有人能确认。其中复位(4)与
    // 咪头(8)连指令都不发(noCommand),纯靠工人现场判断。指示灯拆红/蓝/闪三条、
    ...
    readonly property var manualChecks: [
        ...
        // 复位按键只验**按键本身**(用户定稿 2026-08-18):不验它触发的功能
        //(SIM 卡切换/复位/开关机)——那些是别的测试项的事,在这里按会打断产测流程。
        // 因此不发指令(noCommand),工人按一下确认手感即可,与咪头同类。
        { key: "reset_key", item: 4, group: "复位按键", short: "", noCommand: true,
          label: "按一下复位键，确认按下有回弹、无卡滞（只验按键，不验复位功能）",
