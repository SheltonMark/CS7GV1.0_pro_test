...
QtObject {
    ...
    // ⚠️ 运行时值,禁止硬编码(规则4)。这里只是读不到设备上报时的兜底。
    // =0x6F5:指示灯(0)/白光(2)/复位按键(4)/电池(5)/云台(6)/喇叭(7)/4G(9)/SD(10)。
    // 无 bit1(红外)与 bit3(日夜) —— 本产品无此硬件。
    // 云台(6)=设备端桩执行体恒回成功(无电机);复位按键(4)=按键检测布防,按住
    // 3 秒只上报不复位(libsys 按成品标志位分流)。咪头(8)不在其中:不发指令
    //(喊话→拉流回传→PC 听),不查能力位,见 manualChecks.noCommand。
    readonly property int supportedItems: 0x6F5

    // ---- 人工判定条目全集(8 条) ----
    // 人工项位号(指示灯0/白光2/复位4/云台6/喇叭7/咪头8):发光/出声/转动
    // 固件感知不到,设备最多回"指令已执行",现象只有人能确认。两个特例:
    // 咪头(8)不发指令(noCommand,喊话→拉流回传→PC 听);复位(4)反过来——
    // 人按键、设备检测、按 Code 自动判(deviceJudged)。指示灯拆红/蓝/闪三条、
    ...
    readonly property var manualChecks: [
        ...
        // 复位按键 = 人动作 + 设备判定(用户定稿 2026-08-18):下发布防 → 工人按住
        // 3 秒 → 设备检测到按键事件上报 → PC 按 Code 自动判,窗口期外一律判失败。
        // 只验按键不触发复位:设备端 libsys 按加密分区"成品标志位"分流——未置位
        //(产测中)按 3 秒只上报不复位;已置位(成品)走原复位流程。
        { key: "reset_key", item: 4, group: "复位按键", short: "", deviceJudged: true,
          label: "按住复位键 3 秒，设备检测到按键事件即通过（产测中不会触发复位）",
          runningText: "请按住复位键 3 秒，设备检测中…",
          glyph: Icons.button, op: 0, p1: 0, p2: 0 }
    ]
